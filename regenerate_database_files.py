...
def is_upper_alpha_snake_case(s):
    return bool(re.match("^[A-Z_]+$", s))

def is_safe_name(s):
    return bool(re.match("^[A-Za-z '/_-]+$", s))
...
def parse_skills(j):
    ...
    for (skill_id, skill_json) in j.items():
        ...
        # ID
        if not is_upper_alpha_snake_case(t["skill_id"]):
            raise ValueError("IDs must be upper case letters or underscores.")
        elif t["skill_id"] in skills:
...
def parse_setbonuses(j):
    ...
    for (sb_id, sb_json) in j.items():
        ...
        # ID
        if not is_upper_alpha_snake_case(t["sb_id"]):
            raise ValueError("IDs must be upper case letters or underscores.")
        elif t["sb_id"] in setbonuses:
            raise ValueError("IDs must be unique.")
