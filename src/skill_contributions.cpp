...
double calculate_non_elemental_boost_multiplier(const Database::Database& db, const SkillMap& skills) {
    const unsigned int neb_lvl = skills.get_lvl(db.non_elemental_boost_ptr);
    if (neb_lvl == 0) {
        return 1.0;
    } else {
        assert(neb_lvl == 1);
...
double calculate_raw_crit_dmg_multiplier(const Database::Database& db, const SkillMap& skills) {
    const unsigned int crit_boost_lvl = skills.get_lvl(db.critical_boost_ptr);
    switch (crit_boost_lvl) {
